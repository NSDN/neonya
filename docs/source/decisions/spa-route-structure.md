...
重构版喵玉殿论坛为 SPA 架构。论坛的层级结构为版块（plate）→ 帖子（topic）两级。版块分为普通文字版块（article）和漫画版块（comic），文字版块以标题列表显示帖子，漫画版块以缩略图形式显示帖子。
...
- **方案 B：同一路由 + 条件渲染**（`/plates/:id` 内根据数据的 `pageType` 切换组件）
...
1. **路由结构**：版块列表统一用 `/plates/:id`，帖子详情统一用 `/topics/:id`（待建）。
   组件内部根据数据的类型字段（`pageType` / `topicType`）条件渲染不同视图。
